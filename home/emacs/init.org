#+TITLE: init.org: Emacs Configuration
#+AUTHOR: miyatarutyyy
#+PROPERTY: header-args:emacs-lisp :tangle ./init.el :results silent

* 【前文】
前文だなんて日本国憲法でしか聞いたことがなかったから久しぶりに聞いたんだぜ
おっと、これより前に設定を書くんじゃねぇぞ
...
* 【隔離】
M-x customize とかで勝手に custom-set-variables を init.el に書き込まれるのはいや〜んよぉ
逃がしちゃうわぁ〜ん
...
* 【荷物】
Emacs Lisp packages は Nix ではなく Emacs 側で管理する。
straight.el と use-package に任せる。
...
* 【顕示】
何をしているのか、Discord に晒す。

Discord は nixpkgs のネイティブ版なので、IPC ソケット
（=$XDG_RUNTIME_DIR/discord-ipc-0=）にそのまま届く。
Discord を起動していない間は、elcord が定期的に再接続を試みるだけで害はない。
...
* 【雛形】
org ファイルは org ファイルから生まれる。
Markdown の入り込む余地などない。
...
* 【記事】
Zenn も Qiita も org ファイルで書いちゃうんごねぇ
#+BEGIN_SRC emacs-lisp
...
* 【発表】
MarpもSlidevもなにかちがう
やっぱり org ファイルから beamer を経由して pdf スライドを作るに限る
